...
const SubjectiveTestScreen = ({ user, onComplete }) => {
    ...
    useEffect(() => {
        fetch(`https://speed-to-prof-2.onrender.com/api/questions/subjective/${user.email}`)
            .then(res => res.json())
            .then(data => {
                setQuestions(data);
                setLoading(false);
    ...
    const handleEvaluate = async () => {
        ...
        setEvaluating(true);
        try {
            const res = await fetch('https://speed-to-prof-2.onrender.com/api/evaluate-subjective', {
                method: 'POST',
