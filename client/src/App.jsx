...
const App = () => {
    ...
    const handleLogin = (data, navigate) => {
        ...
        fetch(`https://speed-to-prof-2.onrender.com/api/questions/self-rating/${data.email}`)
            .then(res => res.json())
            .then(q => setScenarioData(p => ({ ...p, self_q: q })));
        fetch(`https://speed-to-prof-2.onrender.com/api/questions/test/${data.email}`)
            .then(res => res.json())
            .then(q => setScenarioData(p => ({ ...p, test_q: q })));
        fetch(`https://speed-to-prof-2.onrender.com/api/questions/subjective/${data.email}`)
            .then(res => res.json())
            .then(q => setScenarioData(p => ({ ...p, subjective_q: q })));
        fetch(`https://speed-to-prof-2.onrender.com/api/assessment-sources/${data.email}`)
            .then(res => res.json())
    ...
    const submitAssessment = (navigate) => {
        fetch('https://speed-to-prof-2.onrender.com/api/submit-assessment', {
            method: 'POST',
            ...
            .then(data => {
                ...
                fetch(`https://speed-to-prof-2.onrender.com/api/dashboard/${currentUserEmail}`)
                    .then(res => res.json())
                    .then(d => {
                        setScenarioData(p => ({
                            ...p,
